...
const DetailsPage = () => {
  ...
  const { data} = useQuery({
    queryKey: ['details'],
    queryFn: () => fetchDetails(`/${params?.explore}/${params?.id}`),
  });
  console.log(data)
  const { data: castData, isLoading: creditLoad, isError: creditErr } = useQuery({
    queryKey: ['cast'], 
    queryFn: () => fetchDetails(`/${params?.explore}/${params?.id}/credits`), 
  })

  const { data : similarData } =useQuery({
    queryKey:['similar'],
    queryFn:()=> fetchData(`/${params?.explore}/${params?.id}/similar`)
  })
  const { data : recommendationData } = useQuery({
    queryKey:['recommendataion'],
    queryFn:()=>fetchData(`/${params?.explore}/${params?.id}/recommendations`)
  })
  console.log(similarData,recommendationData)
